...
from rl.util import clone_model, get_soft_target_model_updates, AdditionalUpdatesOptimizer, huber_loss, clone_optimizer
...
#        critic = Model(inputs=[action_input, obs_input], outputs = q_value) 
        ...
        obs_input_actor = Input(shape=(1,) + env.observation_space.shape, name='observation_input')
        x_ac = Flatten()(obs_input_actor)
        x_ac = Dense(units=256, activation='relu')(x_ac)
        
        obs_input_critic = Input(shape=(1,) + env.observation_space.shape, name='observation_input')
        ...
        critic = Model(inputs=[action_input, obs_input_critic], outputs = q_value) 
...
#        actor_optimizer = SGD(lr=1e-3, momentum=0.9)
        ...
        self.processor=None
        self.random_process = OrnsteinUhlenbeckProcess(theta=.15, mu=0., sigma=.3, size=self.nb_actions)
        self.eps = 0.9
...
#                self.action_exploration=False
    ...
    def backward(self, reward, terminal=False):
        ...
        if can_train_either and self.step % self.train_interval == 0:
            ...
            if self.step > self.nb_steps_warmup:
                #Actor
                target_q_values1 = self.target_critic.predict_on_batch(state1_batch_with_action).flatten()
...
class MACEDDPG(KerasRLAgent):
    def __init__(self,
        ...
        memory = SequentialMemory(limit=1000000, window_length=1)
        random_process = OrnsteinUhlenbeckProcess(size=nb_actions, theta=.15, mu=0., sigma=sigma)
        agent = MACEDDPGAgent(env, nb_actions=nb_actions,
                          memory=memory, nb_steps_warmup_critic=1000, nb_steps_warmup_actor=1000,
                          random_process=random_process, gamma=.99, target_model_update=1e-3, batch_size=64, train_interval=4)
        agent.compile([Adam(lr=.0001, clipnorm=1.), Adam(lr=.0001)], metrics=['mae'])
        self.agent = agent
...
class MACEDDPGAgent(Agent):
    """Write me
    """
    def __init__(self, env, nb_actions, memory,
                 gamma=.99, batch_size=32, nb_steps_warmup_critic=1000, nb_steps_warmup_actor=1000,
                 train_interval=1, memory_interval=1, delta_range=None, delta_clip=np.inf,
                 random_process=None, custom_model_objects={}, target_model_update=.001, **kwargs):
        super(MACEDDPGAgent, self).__init__(**kwargs)
...
#        x = Dense(units=256, activation='relu')(x)
        ...
        for i in range(self.nb_actors):
            ...
            actors.append(actor)
            value = Dense(units=1)(x_critic)
            critic = Model(inputs=[action_input, obs_input_critic], outputs = value) 
